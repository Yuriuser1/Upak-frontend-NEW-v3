...
const tariffPlans: TariffPlan[] = [
  {
    ...
    price: 349,
    features: [
      'Структура карточки после согласования',
      'Уникальные описания от нейросети',
      'Соответствие требованиям WB/Ozon',
      'PDF/DOCX для согласования',
      'Техническое задание для визуала',
...
export function OrderForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!session?.user) {
      toast.error('Пилотные заказы сейчас оформляются через согласование');
    ...
    >
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">
          Заполните форму для подготовки структуры карточки товара
        </p>
      </div>
